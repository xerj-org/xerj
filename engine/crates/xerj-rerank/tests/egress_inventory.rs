...
const KNOWN: &[(&str, Role)] = &[
    ...
    // The local judge's cross-encoder comes from the same Hub. It is a model
    // download and nothing else: the scoring itself never opens a socket,
    // which is the whole point of the `local` provider.
    (
        "xerj-ai/src/seqcls.rs",
        Role::Node("**Local judge model download**"),
    ),
    (

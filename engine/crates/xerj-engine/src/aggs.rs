...
/// itself (not back through this function) and each level consumes at least
/// the one `?` it split on, so a source with at most `MAX_EXPR_DEPTH`
/// ternaries can descend at most `MAX_EXPR_DEPTH + 1` frames.

...
fn malformed_document_reason(
    document_id: Option<&str>,
    item_index: usize,
    error: &serde_json::Error,
) -> String {
    let document_id = document_id.unwrap_or("<auto-generated>");
    format!(
        "invalid document JSON for document ID {document_id:?} at bulk item position {}: \
         parser error: {error}; parser location: line {}, column {}. \
                            ...
                            serde_json::from_slice::<serde::de::IgnoredAny>(dl.as_bytes())
                        {
                            return ParseOutcome::Err(
                                BulkItemResult {
                                    action: action_type.to_string(),
                                    index: target_index,
                                    id: doc_id.clone().unwrap_or_default(),
                                    status: 400,
                                    result: None,
                                    error: Some(malformed_document_reason(
                                        doc_id.as_deref(),
                                        *item_index,
                                        &e,
                                    )),

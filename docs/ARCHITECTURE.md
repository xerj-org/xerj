...
| `xerj-rerank` | Optional second-stage reranking for `_search`: calls an external relevance judge after hits are rendered. The one component that makes an outbound call with document text; never linked by `xerj-engine`. See [RERANK.md](./RERANK.md). |

...
If the tools do not appear: check that the `command` path is absolute and exists, that the node answers the health check in section 3, and that the session was restarted. `xerj mcp` writes diagnostics to stderr. A tool result that says `HTTP 401 Unauthorized` means `XERJ_AUTH` is missing or wrong. An `xerj_search` result over a `semantic_text` field starts with `_xerj.hints` code `lexical_on_semantic_text`: those hits were scored by BM25, whatever the embed mode.

...
- **Honest benchmarking as policy.** The published head-to-head vs live Elasticsearch 8.13.4 reports wins *and* losses with root causes (currently 55W / 26T / 4L, 3 N/A on the full matrix; 1.72× ingest and 1.61× disk wins; the 4 losses are all read-under-write p99 cells, documented). Numbers are only published after an independent adversarial re-measure — and when re-measurement moved the number *against* XERJ, the public number was corrected the same day.

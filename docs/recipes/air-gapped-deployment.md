...
| Rerank provider | Inert: no key is configured | A hosted rerank provider is the only search-time feature that POSTs document text to a third party (the in-process `local` provider, row below, POSTs nothing). The external proxy and WAL tap rows above also send text off the node when enabled; the neural model download and the cluster transport carry no document text; all of them are off by default. It stays inert with no `rerank.api_key` and no `TYPESAFE_API_KEY` in the service environment, and a search must carry a `rerank` block to trigger it. Set `[rerank] enabled = false` to refuse it outright, whatever the environment holds; `GET /_xerj/rerank` confirms `"enabled": false`. See [RERANK.md](../RERANK.md#every-way-data-leaves-a-xerj-node) for every outbound connection a node can open |
| Local judge | Off: `"provider": "local"` is opt-in per request (compiled into the stock release, loaded only when a request asks for it) | Scoring runs in-process and sends nothing. The one network use is the first-load model download from huggingface.co; stage the cross-encoder under `[judge] model_dir` (`rerank-<tier>/config.json`, `tokenizer.json`, `model.safetensors`) and set `[judge] download = false` so a missing file fails loudly instead of reaching for the Hub. `[judge] enabled = false` refuses it outright — note that `[rerank] enabled = false`, which forbids egress, does not, because there is none |

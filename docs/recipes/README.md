...
| [Index your mail: Takeout / Thunderbird / Apple Mail mailboxes](./mail-takeout-mbox.md) | `xerj autoindex` over an unzipped Google Takeout — the mbox streamed and split on a thread pool, attachments (PDF pages) as their own records, Gmail thread ids, `replies_to` / `attachment_of` edges from mail headers; measured on a synthetic mailbox | built into the `xerj` binary — generator in [`scripts/synthetic-takeout.py`](../../scripts/synthetic-takeout.py) |
...
| [Give your agent a second brain it can cite](./second-brain-agent.md) | `xerj brain <folder>` → a bi-temporal, evidence-carrying link index, then the agent loop over four MCP tools: orient, inspect, assert (with a quote), retire — every past moment replayable via `as_of` | built into the `xerj` + `xerj-mcp` binaries — no script needed |
...
| [Vector quantization](./vector-quantization.md) | Opt a `dense_vector` field into scalar8 (`int8_hnsw`) — kNN scored from 1-byte-per-dim codes at recall@10 ≈ 0.998 (precision, **not** memory: see [#392](https://github.com/xerj-org/xerj/issues/392)) | [`quant_demo.py`](../examples/vector-quantization/quant_demo.py) |

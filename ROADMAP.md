...
- **Mail ingest.** *Today on `main`, unreleased:* `xerj autoindex` extracts `.eml` / MIME messages — headers, body, and attachments indexed as their own records ([#921](https://github.com/xerj-org/xerj/pull/921)). *In flight:* branch `feat/ingest-mbox-takeout` — streaming `mbox`, Google Takeout archives, and thread reconstruction. Not on `main`.

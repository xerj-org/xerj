...
- **Judged search — a rerank stage.** Retrieve with BM25 or `hybrid`, then have a second stage re-judge the top *N* before the page is returned. *Today on `main`:* the ES `rescore` block (query rescorer) and the `hybrid` query type with `rrf|linear`; there is no rerank stage. *In flight:* branch `feat/rerank-stage` — a `rerank` block on `_search` that hands the top hits to an external relevance judge, opt-in per request and inert until an operator configures a key. A *hosted* judge would be the only *search-time* feature that sends document text off the machine — proxy embeddings (`[embedding] default_endpoint`) send it at write time and the WAL tap replays writes to an external `_bulk` endpoint, both operator-configured and off by default ([every outbound connection](./docs/RERANK.md#every-way-data-leaves-a-xerj-node)) — which is exactly why a *local* judge is on this list. *The bar for a local model is a measured one.* On the BEIR test splits with the opt-in neural embedder (`--embed-mode neural`, all-MiniLM-L6-v2, CPU) the shipped `hybrid` RRF query is already the best arm we have — nDCG@10 **0.699–0.704 on SciFact and 0.345 on NFCorpus** over three runs, against 0.657 / 0.302 for BM25 alone, 0.676 / 0.329 for vectors alone, and 0.686 / 0.332 for "BM25 top-30 re-ordered by the same bi-encoder", which is *worse* than hybrid on both. So re-ordering with the embedder we already ship is not worth building, and **a local cross-encoder ships only if it beats 0.699 / 0.345 on the same harness, by more than the run-to-run spread** ([`benchmarks/neural-path-triage/`](./benchmarks/neural-path-triage/)). Those figures are with the neural embedder; the default embedder is lexical feature hashing and was not part of this run. The hybrid figure is the only arm that does not reproduce — 0.6993, 0.7023 and 0.7044 on SciFact across three runs of unchanged indices, and 0.7021, 0.7034, 0.7019 across three more — because tied RRF scores are ordered by a per-process hash seed ([#940](https://github.com/xerj-org/xerj/issues/940)); a gain inside that 0.0051 band is not a gain. **ANSWERED (2026-09-20, branch `feat/local-judge`): the bar was not cleared, so the local judge ships opt-in rather than as a default.** An in-process cross-encoder over the hybrid top-30 scored nDCG@10 0.6936 SciFact / 0.3597 NFCorpus / 0.3751 FiQA with the `small` tier (`ms-marco-MiniLM-L6-v2`) — two gains whose 95% paired-bootstrap intervals exclude zero, one wash — and 0.7186 / 0.3328 with `base` (`bge-reranker-base`), which is the best SciFact arm measured and *worse than the smaller model* on NFCorpus. No configuration beat hybrid on every dataset, one 30-document call costs seconds of CPU, and no shipped tier's score is calibrated (`score_kind: "relevance"`). Over a **BM25-only** first stage it is a large win on all three datasets, which is the case it ships for. Full tables, intervals and method: [`benchmarks/local-judge/`](./benchmarks/local-judge/) and [docs/RERANK.md](./docs/RERANK.md#the-local-provider-a-cross-encoder-in-this-process).

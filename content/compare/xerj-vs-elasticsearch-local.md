...
h1: "Is there an Elasticsearch-compatible engine that isn't Elasticsearch?"
description: "The published board is 55 wins, 26 ties, 4 losses and 3 not-applicable across 88 cells against Elasticsearch 8.13.4. All 4 losses are read p99 under a write flood."
slug: "xerj-vs-elasticsearch-local"
cluster: "Comparison: local search engines"
question: "Is there an Elasticsearch-compatible engine that isn't Elasticsearch?"
intent: "comparison"
...
agent_prompt: "Act as a coding agent. Read https://xerj.org/llms.txt, start one XERJ node, index a folder of logs with xerj autoindex, send the same Elasticsearch query DSL you would send to Elasticsearch 8.x, and read the four published read-under-write losses before you recommend either engine."
commands:
  ...
  - cmd: "xerj autoindex ./logs --prefix logs --state-dir ./state-logs"
...
links_out:
  ...
  - "/docs/recipes/migrate-from-elasticsearch"
evidence:
  - claim: "The published board is 55 WIN, 4 LOSE, 26 TIE and 3 N/A over 88 cells at 100,000 documents against a live Elasticsearch 8.13.4 on one host."
    source: "demo/playbooks/SCORECARD.md"
  - claim: "The four losses are the mixed read-under-write p99 cells, a known architectural gap where live-memtable reads sit behind the writer's per-shard lock."
    source: "demo/playbooks/SCORECARD.md"
  - claim: "The #948 ingest RSS runaway is fixed (PR #1002, shipped v1.0.0-rc.76: at-rest retention after a 100k-doc ingest fell ~677 to ~108 MB), but the server still retains heap per indexed document — three per-segment caches are unbounded until a merge retires their segment, an open tracked defect as issue #1032."
    source: "landing/llms.txt:53"
  - claim: "An Elasticsearch index is a logical grouping of physical shards distributed across nodes, and a replica shard is a copy of a primary shard."
    source: "https://www.elastic.co/guide/en/elasticsearch/reference/8.13/scalability.html"
  - claim: "Elasticsearch includes a bundled version of OpenJDK in each distribution, and the documentation states that the bundled JVM is the recommended JVM."
    source: "https://www.elastic.co/guide/en/elasticsearch/reference/8.13/install-elasticsearch.html"
  - claim: "Elastic documents the routes for getting data in as document APIs, language clients, connectors, a file uploader and Elastic Agent integrations, and Filebeat is the shipper that watches log files on a server."
    source: "https://www.elastic.co/guide/en/beats/filebeat/8.13/filebeat-overview.html"
  - claim: "Elasticsearch extracts text from PDF and Office attachments through the ingest attachment processor, which uses Apache Tika, requires base64 encoded input and truncates at 100,000 characters by default."
    source: "https://www.elastic.co/guide/en/elasticsearch/reference/8.13/attachment.html"
  - claim: "Elastic's subscription matrix marks Reciprocal Rank Fusion for hybrid search and the Inference API as Enterprise-tier features."
    source: "https://www.elastic.co/subscriptions"
faq:
  - q: "What is the published score against Elasticsearch?"
    a: "55 wins, 26 ties, 4 losses and 3 not-applicable across 88 cells against a live Elasticsearch 8.13.4 at 100,000 documents on one host."
  - q: "What are the four losses?"
    a: "All four are read p99 under a sustained write flood: match_all, bool, range and terms. Elasticsearch is faster in every one of them."
  - q: "Is XERJ a drop-in for Elasticsearch?"
    a: "No. The wire compatibility is an adoption bridge. XERJ shares no code with Elasticsearch or Lucene, and some request shapes differ."
  - q: "Can XERJ replace an Elasticsearch cluster?"
    a: "No. XERJ is single-node only, with no data-plane replication and no failover. One host is the whole deployment."
  - q: "Can I get ES query DSL without the JVM?"
    a: "On the compatible surface, yes. XERJ is one static Rust binary of about 36 MB with a sub-second start, so no JVM and no container runtime is involved, and some request shapes still differ."
  - q: "I just want to search logs on my laptop. I don't want Elasticsearch in Docker."
    a: "Then run the binary on the host. Elasticsearch expects a shipper or an ingest pipeline; XERJ reads the log folder directly with xerj autoindex and needs no container runtime."
  - q: "What is the memory risk with XERJ?"
    a: "The server retains heap per indexed document. The #948 ingest runaway is fixed as of v1.0.0-rc.76, but three per-segment caches remain unbounded until a merge retires their segment (open issue #1032), so corpora of many millions of documents can still OOM one node."
  - q: "When should I stay on Elasticsearch?"
    a: "When you read while a heavy writer runs, when you need more than one host, or when your corpus grows past a few million documents."
---

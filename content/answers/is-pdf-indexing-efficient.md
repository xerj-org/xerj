...
h1: "Is it efficient to index PDF files?"
description: "PDF extraction is the slowest family XERJ parses. XERJ runs it in an isolated worker sized from free memory, caps it 5 ways, and refuses image-only files."
slug: "is-pdf-indexing-efficient"
cluster: "Files and formats"
question: "Is it inefficient to index PDF files into a search engine?"
...
commands:
  - cmd: "xerj autoindex ./pdf --url http://127.0.0.1:9410 --prefix pdfx --state-dir ./state-pdfx --progress plain --disable-feedback"
    note: "Index a PDF folder and record the worker decisions on stderr."
  - cmd: "curl -s -XPOST 'http://127.0.0.1:9410/autoindex-catalog/_search' -H 'content-type: application/json' -d '{\"query\":{\"term\":{\"doc_kind\":\"run\"}},\"size\":10,\"sort\":[{\"started\":\"desc\"}]}'"
    note: "Read the PDF worker count, wall time and extraction reuse counters."
  - cmd: "curl -s -XPOST 'http://127.0.0.1:9410/autoindex-catalog/_search' -H 'content-type: application/json' -d '{\"query\":{\"bool\":{\"filter\":[{\"term\":{\"doc_kind\":\"file\"}},{\"term\":{\"status\":\"junk\"}}]}},\"size\":20}'"
...
faq:
  - q: "Is PDF the slowest format XERJ indexes?"
    a: "Yes. PDF text extraction is the most expensive family, which is why XERJ runs it in a separate worker process and caps it 5 ways."
  - q: "How many PDF workers does XERJ run?"
    a: "As many as free memory allows at 1536 MiB each. Our 16-core host got 1 worker, because the measured memory safe zone was 966 MiB."
  - q: "Does XERJ parse the same PDF twice?"
    a: "No. XERJ keeps the phase A extraction and replays it in phase B. Our run replayed 3 artifacts with 0 integrity failures and 0 second parses."
  - q: "What happens to an image-only PDF?"
    a: "XERJ refuses it with `status=junk` and 0 documents, and the run exits 3 with `reason=completed-with-junk`. The catalog keeps the reason string."
  - q: "Does XERJ run OCR on a PDF?"
    a: "No. XERJ has no optical character recognition. Run OCR outside XERJ, then index the searchable PDF that the OCR step produces."
  - q: "How much memory does one PDF worker use?"
    a: "Our capture did not measure it. XERJ reserves 1536 MiB per worker when it sizes the pool, so use that as the planning number."
  - q: "How large a PDF can XERJ read?"
    a: "512 MiB and 100,000 pages, with 32 MiB of worker output and a 120 s default timeout. A file past a cap is refused, never half-indexed."
  - q: "Will XERJ warn me before a long PDF job?"
    a: "Yes. The decision gate measures an extraction floor first. On 2,500 PDFs with `--max-minutes 1` it exited 4 and wrote a decision request."
---

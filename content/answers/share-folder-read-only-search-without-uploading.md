...
faq:
  ...
  - q: "What if they are not on my network?"
    a: "Add `--tunnel`. XERJ starts your own `cloudflared`, prints a public link, and closes the tunnel and revokes the share when you press Ctrl-C."
...
The folder path was run end to end on 2026-09-18 on a Linux host: `xerj brain` on a folder of 3 files (two notes and one email), `xerj share` on the same folder, a claim with the printed passcode, a guest search, and a walk of the folder's links. The same run confirmed that the guest key was refused the catalog index, `_cat/indices` and `/_share`. macOS and Windows were not run.

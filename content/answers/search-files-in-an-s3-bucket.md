...
h1: "How do I search files that live in an S3 bucket?"
description: "Point xerj autoindex at s3://bucket/prefix. It lists the prefix, streams only the objects whose ETag changed, and indexes them. One LIST request per 1,000 keys."
slug: "search-files-in-an-s3-bucket"
cluster: "Ingest: object storage"
question: "How do I search files that live in an S3 bucket?"
...
evidence:
  - claim: "xerj autoindex accepts s3:// and r2:// wherever it accepts a folder; the prefix is listed with ListObjectsV2 at 1,000 keys per page and each admitted object is streamed in."
    source: "engine/crates/xerj-autoindex/src/objsource.rs"
  - claim: "Change identity is the object's ETag plus its size, treated as an opaque token: a multipart ETag's -N suffix is used verbatim and nothing is compared to a locally computed MD5."
    source: "engine/crates/xerj-autoindex/src/objsource.rs"
  - claim: "Measured against MinIO: first run over 4 keys = 1 LIST + 3 GET; unchanged re-run = 1 LIST + 0 GET, 0 bytes; 1,200 keys = 2 LIST pages; a 1 GiB object moved in 692 ms with RSS growing from 25 MB to 29 MB."
    source: "engine/crates/xerj-autoindex/src/objsource_minio_tests.rs"
  - claim: "The index still lives on the XERJ node's local disk. Setting storage.backend to s3 refuses to start, and no segment or WAL file is ever written to a bucket."
    source: "docs/OBJECT_STORAGE.md:206"
  - claim: "Keys whose components are hidden, unsafe or unportable are skipped by name and reported: a bucket's .env and .git/ never reach the index."
    source: "engine/crates/xerj-autoindex/src/source.rs"
faq:
  - q: "How do I search files that live in an S3 bucket?"
    a: "Run `xerj autoindex s3://bucket/prefix` against a running node. It lists the prefix, streams each object into a local mirror under the state dir, and indexes the documents. Credentials come from the environment."
  - q: "Does it download the whole bucket every time I run it?"
    a: "No. Each object's ETag and size are recorded in the state dir, and a re-run downloads only what changed. Measured against MinIO, an unchanged prefix costs 1 LIST request and zero GETs."
  - q: "Will this blow through my R2 or S3 free tier?"
    a: "A run makes no writes, so its only class-A cost is listing: one request per 1,000 keys. Daily or hourly over a normal corpus is a fraction of a percent. Every run prints what a schedule would cost."
  - q: "Can I use MinIO, Ceph or Cloudflare R2?"
    a: "Yes. Pass `--endpoint-url`, or set AWS_ENDPOINT_URL_S3. R2 needs the account host because there is no default for it. The bucket is then addressed in the path rather than as a subdomain."
  - q: "Does the index live in the bucket?"
    a: "No, and that matters for sizing. The objects are mirrored to local disk under the state dir and the index is written to the XERJ node's own disk. Keeping the index in object storage is not implemented."
  - q: "What happens to search results when someone deletes an object?"
    a: "Run with `--no-graph`. The next run removes the object from the local mirror and the generated journal's incremental reconcile deletes its documents. On the default graph-enabled journal a deletion is refused instead, exactly as it is for a folder, and the run tells you how to rebuild."
  - q: "Is a multipart-uploaded object handled correctly?"
    a: "Yes. A multipart ETag is a digest of digests with a `-N` suffix, so it is not an MD5 of the body. XERJ never treats it as one: it stores the string and asks only whether it changed."
  - q: "My bucket has 40,000 objects. What does one run cost?"
    a: "40 LIST requests, plus one GET per changed object, plus disk for the bytes. Hourly that is 28,800 class-A requests a month, under 3% of a 1,000,000 free allowance for that prefix."
---

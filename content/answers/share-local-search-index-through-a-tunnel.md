...
faq:
  ...
  - q: "Can Cloudflare read what my guest searches for?"
    a: "It terminates the HTTPS connection, so in principle yes. Nothing is uploaded or stored there, but the results your guest requests cross its network. Use your own hostname and certificate with `--public-url` when that matters."
...
To record each guest's real address in the audit log, list loopback under `trusted_proxies` in the `[server]` section of the node's config and restart it. Only then is the forwarded header read. The command tells you which mode your node is in.

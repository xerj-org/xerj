...
commands:
  ...
  - cmd: "curl -s -XPOST 'http://127.0.0.1:9200/_share/0123456789abcdef0123456789abcdef/claim' -H 'content-type: application/json' -d '{\"passcode\":\"abcd-2345\"}'"
    note: "What the guest page does: exchange the share id and the passcode for a read-only key. This id and passcode are placeholders and return 404."
...
links_out:
  ...
  - "/docs/security"
evidence:
  - claim: "The guest route allow-list: _search, _count, _msearch, _mget, _mapping, _field_caps, GET _doc and _source, and the graph ego and overview routes."
    source: "engine/crates/xerj-api/src/authz.rs"
  - claim: "The live test makes 193 checks against a node with authentication on; all 193 passed on 2026-09-18."
...
faq:
  ...
  - q: "What is stored on disk about a share?"
    a: "A SHA-256 digest of the share id and an Argon2id hash of the passcode, in `shares.json` with mode 0600. No share id, passcode or guest key is written to disk or to the audit log."
...
**TL;DR** — A share-link guest gets a key with `read` on the shared index and nothing else. A route allow-list, used for guests only, closes `_cat`, `_cluster`, every other index, all writes, and scroll and point-in-time contexts. Each refusal is a check in a live test that ran 193 checks on 2026-09-18, with 0 failures.
...
| `GET _doc/{id}`, `GET _source/{id}` | the shared indices |
...
`POST /_share/{id}/claim` is the one route on the node that hands out a credential without authentication, so it is narrow. It is `POST` only, it is never cached, and every outcome goes to the audit log with the source address.

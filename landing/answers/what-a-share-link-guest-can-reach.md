<!-- generated by build_articles.py — edit content/answers/what-a-share-link-guest-can-reach.md instead -->
---
...
# If I share one index with a link and a passcode, what can that person actually reach on my node?

**TL;DR** — A share-link guest gets a key with `read` on the shared index and nothing else. A route allow-list, used for guests only, closes `_cat`, `_cluster`, every other index, all writes, and scroll and point-in-time contexts. Each refusal is a check in a live test that ran 202 checks on 2026-09-18, with 0 failures.
...
```sh
xerj share casefile --label 'for the auditor' --max-claims 1
```
...
```sh
curl -s -XPOST 'http://127.0.0.1:9200/_share/0123456789abcdef0123456789abcdef/claim' -H 'content-type: application/json' -d '{"passcode":"abcd-2345"}'
```
...
```sh
xerj share --revoke 8a1e546d4ee8
```
...
### If I give someone a read-only share link to one search index, can they see my other indices or anything else on the server?

No. The guest key is granted `read` on the shared index only, and a route allow-list closes everything that describes the node. Another index is a 403 by name, by alias and from inside a request body.

### Can a guest list my indices with _cat/indices?

No. `_cat`, `_cluster`, `_nodes`, `_snapshot`, `_tasks` and `_stats` all return 403 to a guest key. An ordinary scoped key sees a filtered view of those; a guest sees none of it.

### Can a guest write, delete or reindex?

No. Index, update, delete, bulk, delete-by-query, reindex, refresh, mapping and settings changes all return 403. The live test also confirms that no guest write landed.

### Can a guest keep a scroll or a point-in-time open on my machine?

No. `scroll`, `_search/scroll`, `_pit` and `_async_search` are refused, and so is a `pit` clause in a search body, so a guest cannot ride a point-in-time that someone else opened.

### What is stored on disk about a share?

A SHA-256 digest of the share id and an Argon2id hash of the passcode, in `shares.json` with mode 0600. No share id, passcode or guest key is written to disk or to the audit log.

### What can a guest still do that I should think about?

Read every document in the shared index and copy what they read. A share grants whole indices. There is no per-document or per-field restriction to apply.

## Evidence

- The guest route allow-list: _search, _count, _msearch, _mget, _mapping, _field_caps, GET _doc and _source, and the graph ego and overview routes. — `engine/crates/xerj-api/src/authz.rs`
- The live test makes 202 checks against a node with authentication on (192 without the native-listener section); all passed on 2026-09-18. — `docs/usecases/share-links/VERIFICATION.md`
- A share id is 128 bits and is stored as a SHA-256 digest; the passcode is stored as an Argon2id hash in a file with mode 0600. — `engine/crates/xerj-api/src/share.rs`
- Unknown share ids are limited to 10 a minute and 100 an hour per source address; a known share to 10 a minute and 30 an hour. — `engine/crates/xerj-api/src/share.rs`
- The guest page is served with default-src 'none' and frame-ancestors 'none'. — `engine/crates/xerj-console-api/src/spa.rs`

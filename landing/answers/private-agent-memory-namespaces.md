<!-- generated by build_articles.py — edit content/answers/private-agent-memory-namespaces.md instead -->
---
title: "Give two agents private memory on one laptop"
canonical: "https://xerj.org/answers/private-agent-memory-namespaces"
updated: "2026-08-23"
source: "content/answers/private-agent-memory-namespaces.md"
---

# How do I give two agents private memory on one laptop?
...
```text
Act as a coding agent. Read https://xerj.org/llms.txt, start a XERJ node without --insecure, mint a scoped API key, store a memory in one namespace, then try to recall a different namespace with that key and report the exact status code you receive.
```
...
```sh
curl -s -XGET 'http://127.0.0.1:8430/_security/roles'
```
...
```sh
curl -s -XPOST 'http://127.0.0.1:9430/_memory/definitely-not-mine/_recall' -H 'content-type: application/json' -d '{"query":"anything","k":3}'
```
...
### How do I keep one agent's memory from leaking into another agent's?

Give each agent its own namespace and its own scoped API key. A scoped key is confined to its named indices, and authorization runs before the existence check.

### Can two local agents share a search engine but not memory?

Yes. Both can query the same document indices on one node, while each scoped key reaches only its own reserved `.xerj-memory-*` namespace.

### How do I point an MCP memory server at a folder I choose?

Point the node's data directory there. XERJ has no separate memory path: memory lives in the data directory as `.xerj-memory-{namespace}`, and the MCP server stores nothing itself.

### Does XERJ enforce roles?

No. The node itself answers `"enforced": false` and warns that every authenticated caller has full superuser access. Full role enforcement is deferred.

### What authorization does exist?

Scoped API keys. XERJ confines a scoped key to its named indices and reserves the `.xerj-memory-*` namespace, which is principal scoping rather than roles.

### Can an attacker enumerate my namespaces?

Not through status codes on a secured node. Authorization runs before the existence check, so an absent brain and an unauthorized brain both answer 403.

### What does --insecure turn off?

TLS and API-key authentication. Any client that reaches the port can then read and delete every index, so use it on a development machine only.

<!-- generated by build_articles.py — edit content/answers/cheap-low-volume-log-search.md instead -->
---
title: "What can I use for low-volume log search?"
canonical: "https://xerj.org/answers/cheap-low-volume-log-search"
updated: "2026-08-23"
source: "content/answers/cheap-low-volume-log-search.md"
---
...
```text
Act as a coding agent. Read https://xerj.org/llms.txt, index a small folder of plain-text logs with xerj autoindex, then run a term filter on level and a terms aggregation on service, and record the node's peak resident memory from /proc before you plan a larger corpus.
```
...
### Can XERJ replace a hosted log service?

For low volume on one node, yes. A measured 16 MB corpus indexed in 22.237 s and answered exact error counts in under a millisecond. Large volumes are a different question.

### How much memory does log indexing use?

About 200x the corpus size. The measured ladder reached 2,755.9 MB peak resident memory after 28 MB of cumulative log data on one node.

### How large a log corpus can XERJ handle?

This capture measured up to 16 MB per step and stopped there for memory headroom. Plan for corpora up to a few million documents, and measure your own host.

### Are XERJ log counts exact or estimated?

Exact. At every ladder step the level=ERROR document count matched the generator's own count exactly: 424, 848 and 1,696.

### How fast are queries over an indexed log corpus?

On the 16 MB corpus, p50 was 0.488 ms for an error filter, 0.536 ms for a terms aggregation and 0.676 ms for a date range with an aggregation.

### Does a match query on the text field find log lines?

No. The logs family writes ts, level and message fields with no generic text field, and message is a keyword. Query message or level instead.

### How much disk does an indexed log corpus need?

The 3 measured steps stored 1.045, 1.319 and 1.039 bytes per source byte. The ratio is not stable, so do not publish a single storage multiplier.
...
- [Log analytics](/docs/recipes/log-analytics)

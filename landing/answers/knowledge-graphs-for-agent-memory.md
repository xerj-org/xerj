<!-- generated by build_articles.py — edit content/answers/knowledge-graphs-for-agent-memory.md instead -->
---
title: "Should agent memory use a graph?"
canonical: "https://xerj.org/answers/knowledge-graphs-for-agent-memory"
updated: "2026-08-23"
source: "content/answers/knowledge-graphs-for-agent-memory.md"
---

# Should agent memory use a knowledge graph?
...
```sh
xerj autoindex ./notes --url http://127.0.0.1:9430 --prefix br --state-dir ./state-br --brain notes --progress plain
```
...
```sh
curl -s -XGET 'http://127.0.0.1:9430/_graph/notes/overview'
```
...
```sh
curl -s -XGET 'http://127.0.0.1:9430/_graph/notes/ego?node=0ddfc0b862bd2b7ee0b308b8ce4768c9&hops=2&include_nodes=true'
```
...
### Does XERJ give me a graph database for agent memory?

No. XERJ builds a bounded link index over its own indexed documents. It has no Cypher, no shortest-path and no variable-depth traversal.

### How many hops can I traverse?

One or two. A 3-hop request returns HTTP 400 with the cap stated, and the response tells you to expand again from the returned reachable ids.

### What creates the links?

Deterministic detectors. Our run used `wikilink@2`, `sequence@2` and `samedir@2`, which read wikilinks, section order and shared directories.

### Are the detectors reading meaning?

No. Seven of the eight detectors are structural, and the one content-reading detector compares unstemmed strings, so it matches text rather than concepts.

### What are the graph bounds?

Hops cap 2, 4,096 frontier ids per hop, 100,000 result edges per request, 64 ego seeds, 1,000 ego edges and 10,000 restrict ids.

### Can recall use the links?

Yes. Pass a `graph` object to `_recall` with `mode` set to `restrict` or `blend`, plus `seeds`, `hops` of 1 or 2, and an optional `weight`.

### Where do the edges live?

In an index named `.xerj-memory-{brain}-edges`. `autoindex` creates one per indexed folder by default, named after the folder.

## Related

- [How can agent-memory links retain evidence?](/answers/evidence-for-agent-memory-links)

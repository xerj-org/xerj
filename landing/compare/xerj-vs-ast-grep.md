<!-- generated by build_articles.py — edit content/compare/xerj-vs-ast-grep.md instead -->
---
title: "XERJ vs ast-grep — syntax search and an index"
canonical: "https://xerj.org/compare/xerj-vs-ast-grep"
updated: "2026-08-23"
source: "content/compare/xerj-vs-ast-grep.md"
---

# ast-grep vs a code index for an agent?
...
```sh
curl -s -XPOST 'http://127.0.0.1:9200/code-*/_search' -H 'content-type: application/json' -d '{"query":{"match":{"defs":"parse_config"}},"_source":["ax_path"],"size":20}'
```
...
### Is there a syntax-aware code search I can use when refactoring?

Yes, ast-grep. It changes code by syntax pattern, and XERJ answers questions about a tree it has already indexed. They do different jobs, so install both.

### Can XERJ rewrite code?

No. XERJ performs no abstract syntax tree rewriting and no codemod of any kind. It returns passages and paths, and you change the file.

### What does XERJ extract from code?

Definitions. Each code document carries a defs field and a symbols list with names, kinds and line numbers, across 34 languages.

### Does ast-grep keep an index?

No. The project states that it builds a syntax tree in memory for each query and does not support offline indexing.

### Which one does an agent call first?

The index, to find the function and read its contract. Then ast-grep, to apply the structural change across every match.

### Does ast-grep have an MCP server?

Yes. Its organization publishes an experimental MCP server with four tools, so agent access is not a place where XERJ stands alone.

### When should a coding agent use ast-grep instead of grep?

When the change is structural. Plain grep stays the right tool for a one-off literal on a tree that changed a second ago, and for a substring inside an identifier, which an analyzed index cannot match.

### Is there a measured comparison on this page?

No. No head-to-head was run. Every ast-grep fact here comes from its own documentation and repository.

## Evidence

- ast-grep is a command-line tool for code structural search, lint and rewriting, and it matches abstract syntax tree nodes rather than text. — [https://github.com/ast-grep/ast-grep](https://github.com/ast-grep/ast-grep)
- ast-grep rewrites either with the --rewrite flag on a run, or with a fix key inside a YAML rule applied by ast-grep scan. — [https://ast-grep.github.io/guide/rewrite-code.html](https://ast-grep.github.io/guide/rewrite-code.html)
- The ast-grep reference lists 26 built-in languages backed by tree-sitter grammars, and custom grammars can be loaded dynamically. — [https://ast-grep.github.io/reference/languages.html](https://ast-grep.github.io/reference/languages.html)
- The ast-grep project states that it builds an abstract syntax tree in memory for each query and does not support offline indexing. — [https://ast-grep.github.io/blog/code-search-design-space.html](https://ast-grep.github.io/blog/code-search-design-space.html)
- The ast-grep tool comparison states that it has no deep semantic information and operates only on the syntactic level, with no type information, control flow, data flow or taint analysis. — [https://ast-grep.github.io/advanced/tool-comparison.html](https://ast-grep.github.io/advanced/tool-comparison.html)
- The ast-grep organization publishes an experimental Model Context Protocol server exposing four tools for structural code search. — [https://github.com/ast-grep/ast-grep-mcp](https://github.com/ast-grep/ast-grep-mcp)
- GNU grep prints lines that contain a match for one or more patterns, which makes it a line-oriented tool rather than a syntax-aware one. — [https://www.gnu.org/software/grep/manual/grep.html](https://www.gnu.org/software/grep/manual/grep.html)

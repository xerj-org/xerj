<!-- generated by build_articles.py — edit content/compare/xerj-vs-docfetcher.md instead -->
---
title: "DocFetcher compared with XERJ for document search"
canonical: "https://xerj.org/compare/xerj-vs-docfetcher"
updated: "2026-08-23"
source: "content/compare/xerj-vs-docfetcher.md"
---

# What's a free DocFetcher alternative that an agent can call?
...
```text
Act as a coding agent on a Windows or Linux workstation with a documents folder. Read https://xerj.org/llms.txt, start a local XERJ node, run xerj autoindex on the folder, read the data map, then answer the user question with a match_phrase query that cites the ax_path of every hit.
```
...
```sh
curl -s -XPOST http://127.0.0.1:9200/docs-*/_search -H 'content-type: application/json' -d '{"query":{"match_phrase":{"body":"purchase order"}},"size":10,"_source":["ax_path","ax_format","title"]}'
```
...
### How do I search a documents folder on Windows without Spotlight?

Index the folder with a tool of your own. DocFetcher indexes it for a person at the keyboard. XERJ indexes it for a program, and answers over HTTP or MCP.

### DocFetcher vs a local search API?

DocFetcher is a GUI application with no API in the free version. A local search API answers a program instead of a person, which is the difference that matters for an agent.

### How do I search a folder of PDFs, Word docs, and markdown all at once?

Both tools index the folder in one pass. DocFetcher shows the hits in a result window. `xerj autoindex` writes typed documents that a `match_phrase` query reads over HTTP.

### Did you run a head-to-head benchmark between XERJ and DocFetcher?
...
### Which tool carries its index on a USB drive?

DocFetcher. The portable versions keep the index in the application folder, so the indexed repository travels with the drive. XERJ keeps its data in the node data directory.

### Can an agent call DocFetcher directly?

Not the free desktop application, which has a GUI and no API. A multi-user web interface is sold separately as DocFetcher Server.

## Evidence

- DocFetcher is an open-source desktop search application under the Eclipse Public License, and it runs on Windows, Linux and macOS. — [https://docfetcher.sourceforge.io/](https://docfetcher.sourceforge.io/)
- DocFetcher portable versions keep the index in the application folder, which makes a fully indexed document repository that can travel on a USB drive or a cloud drive. — [https://docfetcher.sourceforge.io/](https://docfetcher.sourceforge.io/)
- DocFetcher indexes Microsoft Office, Outlook PST, OpenOffice, PDF, EPUB, HTML, RTF, CHM, AbiWord, Visio, SVG and customizable plain-text formats, plus zip, 7z, rar and tar archives with unlimited nesting. — [https://docfetcher.sourceforge.io/](https://docfetcher.sourceforge.io/)
- The DocFetcher query syntax supports OR, AND and NOT with wildcards, phrase search, fuzzy search, proximity search and boosting, and the preview pane highlights every match. — [https://docfetcher.sourceforge.io/](https://docfetcher.sourceforge.io/)
- DocFetcher 1.1.27 requires Windows 7 SP1 or later, Linux with GTK3, or macOS 11 or later, and since 1.1.26 no separate Java runtime install is needed. — [https://docfetcher.sourceforge.io/download/](https://docfetcher.sourceforge.io/download/)
- A multi-user web interface is sold separately as DocFetcher Server; the free desktop application has no such interface. — [https://docfetcher.sourceforge.io/](https://docfetcher.sourceforge.io/)
- ripgrep-all wraps ripgrep with adapters for PDF, Office documents, archives and SQLite, and needs no index. — [https://github.com/phiresky/ripgrep-all](https://github.com/phiresky/ripgrep-all)
- Recoll indexes email and can run OCR on image-only PDF documents through tesseract or ABBYY FineReader. — [https://www.recoll.org/usermanual/usermanual.html](https://www.recoll.org/usermanual/usermanual.html)

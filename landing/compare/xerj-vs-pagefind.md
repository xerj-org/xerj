<!-- generated by build_articles.py — edit content/compare/xerj-vs-pagefind.md instead -->
---
title: "XERJ vs Pagefind for a folder of saved HTML"
canonical: "https://xerj.org/compare/xerj-vs-pagefind"
updated: "2026-08-23"
source: "content/compare/xerj-vs-pagefind.md"
---

# Pagefind vs indexing a local HTML dump for an agent?
...
```sh
curl -s -XPOST 'http://127.0.0.1:9200/site-*/_search' -H 'content-type: application/json' -d '{"query":{"match_phrase":{"body":"rate limit"}},"_source":["ax_path"]}'
```
...
### What is Pagefind for?

Search on a built static website, running in the visitor's browser with no server component and a chunked index that keeps the download small.

### Can Pagefind index a PDF?

Only if you extract the text yourself and pass it through addCustomRecord. The command-line tool discovers HTML files by default.

### I saved a bunch of docs pages as HTML. How do I search that?

Index the folder where it sits. XERJ reads the HTML files and any PDF, DOCX, CSV or SQLite files beside them, then answers an agent over the Elasticsearch REST API.

### Can XERJ put a search field on my website?

No. XERJ is not a static-site plugin and ships no browser bundle. Pagefind is the right tool for that job.

### Is there a measured comparison here?

No. No head-to-head was run. Every Pagefind fact on this page comes from Pagefind's own documentation.

### Do I need a running process?

For Pagefind, no. For XERJ, yes: one single-node process serves the queries.

### What license is Pagefind under?

Pagefind is MIT and XERJ is Apache-2.0. Both are permissive, so neither one constrains what you build on top of it.

### How do I search a saved website or HTML export?

It depends who is asking. Pagefind serves your readers in the browser on a built site, XERJ answers your agent about the same files on disk, and both can run against one folder.

## Evidence

- Pagefind is a fully static search library that runs after a static site generator and adds a search bundle to the built files, and its documentation states that Pagefind itself has no server component. — [https://pagefind.app/docs/](https://pagefind.app/docs/)
- Pagefind splits its index into chunks so that a browser loads only a subset, and it reports a full-text search on a 10,000 page site with a total network payload under 300kB, closer to 100kB for most sites. — [https://pagefind.app/](https://pagefind.app/)
- The Pagefind command-line tool discovers files with a glob that defaults to HTML only. — [https://pagefind.app/docs/config-options/](https://pagefind.app/docs/config-options/)
- Non-HTML content such as PDFs or subtitles enters a Pagefind index through addCustomRecord, where the caller supplies the already-extracted content text. — [https://pagefind.app/docs/node-api/](https://pagefind.app/docs/node-api/)
- Pagefind is published under the MIT license. — [https://github.com/Pagefind/pagefind](https://github.com/Pagefind/pagefind)
...
- [Why does my Notion export duplicate titles?](/answers/notion-export-duplicate-search-results)

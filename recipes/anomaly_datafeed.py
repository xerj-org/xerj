#!/usr/bin/env python3
"""Continuously detect anomalies with an XERJ _ml datafeed.

The on-demand `_score` API (see recipes/log_anomaly.py) scores a series once,
on request. A *datafeed* turns that into a live monitor: it references a
detector, and once started it re-scores the source index every `frequency`,
appending only the NEW flagged buckets to the job's results store — which you
poll with `GET /_ml/anomaly_detectors/{job}/results/records`. Same transparent
moving mean/stddev z-score, now running on a timer.

This recipe proves the "continuous" part end to end:
...
XERJ = os.environ.get("XERJ_URL", "http://localhost:9200").rstrip("/")
INDEX = "cpu_datafeed"
DETECTOR = "cpu-spike"

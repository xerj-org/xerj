/* XERJ head-to-head comparison brief — same SVG-injection pipeline as
   arch-brief, rendered via headless Chrome (no puppeteer). */
...
(async () => {
  const tplPath = path.resolve(__dirname, 'compare-brief.html');
  const svgDir  = path.resolve(__dirname, 'svg');
  const outHtml = path.resolve(__dirname, '_compare-brief.rendered.html');
  const outPdf  = path.resolve(__dirname, 'xerj-compare-brief.pdf');

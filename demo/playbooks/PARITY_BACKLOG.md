...
- [ ] **B — index management ops**: `_close`/`_open`, `_cache/clear`, `_disk_usage`, `_shrink`/`_split`, `_freeze`/`_unfreeze`, `_recovery`
- [ ] **C — cluster/node stats realness**: `_nodes`, `_nodes/stats`, `_nodes/:id/stats`, `_cluster/stats`, `_cluster/state`, `_cluster/pending_tasks`, `_internal/desired_balance`, `_index_template/_simulate_index`, `painless/_execute`, `_ilm/policy`

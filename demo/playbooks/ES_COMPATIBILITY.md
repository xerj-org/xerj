...
- **HNSW-served kNN** — unfiltered, cosine, full-precision kNN on indexes ≥1,024 docs is now served by a persisted HNSW graph with exact rescoring (measured recall@10 1.00 on the bench query; filtered/nested/SQ8/small-index shapes keep the exact brute-force scan). **`scalar8` (`int8_hnsw`)** has a real serving path — scores are computed from 1-byte-per-dimension codes, not from the f32 vector (recall@10 ≈ 0.998, measured) — but it buys **precision, not memory**: the scan reads the full-precision vector from `_source` and quantizes it per query, so nothing gets smaller in RAM. The earlier "~4× smaller vector working set" claim described caches that [#371](https://github.com/xerj-org/xerj/issues/371) removed — both the per-document code map and the per-field codebook were write-once, and between them kept scoring updated documents from stale data. The ingest-time, ordinal-addressed code array that would make a memory claim true is tracked in [#392](https://github.com/xerj-org/xerj/issues/392), which also covers the two consequences of fitting the codebook per query: `scalar8` still disqualifies a field from HNSW-served kNN, and a `scalar8` `_score` varies with the candidate set (bounded by SQ8's own 1/255 quantization step) where Elasticsearch's, fixed per segment at index time, does not. `binary` (1-bit) is still rejected at startup.

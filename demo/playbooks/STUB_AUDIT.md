...
**Update (RC3 / RC4, 2026-07-12 — re-count):** beyond the four silent-fakes
above, several more publicly-claimed gaps have since closed and are marked
**✅** in the tables below — **TLS/HTTPS** (in-process rustls termination),
**`percolate`** (real reverse-search doc-scan), **`has_child` / `has_parent`**
(now fail loud with a `400` instead of returning flat hits), **`scalar8` /
`int8_hnsw` quantization** (real serving path; `binary`/`scalar4`/offload still
open), and **agent-memory dedup + recency-blended recall** (now wired to the
REST path). Ten ✅ rows in all. The ES-compat YAML suite is now green at
**1,360 / 1,363** (it was 1,328 when the four silent-fakes were fixed). The
remaining rows are honest roadmap — tracked in ROADMAP.md, not silently faked.
...
| TLS/HTTPS on the REST + ES-compat listeners (cfg.tls.enabled) | stub | `crates/xerj-server/src/main.rs` | ✅ FIXED (RC3/RC4) — in-process **rustls** TLS termination now wraps the REST + ES-compat listeners (`main.rs` `build_tls_config` → `RustlsConfig::from_pem_file`; startup banner: "TLS: in-process rustls termination active"). Original finding: REAL gap, not a false positive. Verified: serve() (main.rs:332-351) binds a plain tokio::net::TcpListener and runs `axum::serve(listener, router).tcp_nodelay(tr… |
| has_child / has_parent parent-child join queries | stub | `crates/xerj-query/src/planner.rs:633` | ✅ FIXED (25c6b6c) — `has_child`/`has_parent` now **fail loud with a 400** at parse (`parse_has_child`); the unfiltered-hits path is unreachable. Full parent-child joins remain future work, but the silent-wrong-answer is gone. Original finding: Confirmed real gap. The defining join semantics are entirely absent on the normal execution path — both execution paths run the inner query flat and discard the… |
...
| percolate query (POST _search {"percolate":{...}}) | stub | `crates/xerj-query/src/parser.rs` | ✅ FIXED (e72ac71) — real **reverse-search doc-scan**: `parse_percolate` parses inline `document`/`documents`, and `index.rs` `QueryNode::Percolate` parses each doc's stored query and matches the supplied documents against it. Live tests cover hit/miss/bool shapes. Original finding: CONFIRMED real stub. At parser.rs:309 the `percolate` query type is hard-wired to `Ok(QueryNode::MatchNone)` with the inline comment "percolate not supported". … |
...
| Agent memory (/_memory) — semantic dedup + recency-blended recall | partial | `engine/crates/xerj-ai/src/memory.rs` | ✅ FIXED (ec6dc4a) — the REST `/_memory/{ns}/_recall` path now wires **recency-blended scoring** (`recency_weight`) and **semantic dedup** (`dedup`/`dedup_threshold`) via `memory_api.rs` `blend_recency`; the previously-orphaned module is on the request path. Original finding: Real gap, verified in both directions. The AgentMemory module in memory.rs is fully implemented and unit-tested (real cosine dedup, real e^(-age/7d) recency ble… |
...
| has_child / has_parent join queries | partial | `crates/xerj-engine/src/index.rs:13256` | ✅ FIXED (25c6b6c) — parse now rejects `has_child`/`has_parent` with a 400, so this execution path is unreachable (no more flat inner-query hits). Original finding: REAL gap, correctly labeled partial. Traced the full path: parser (parser.rs:3021-3075) fully parses has_child/has_parent into QueryNode::HasChild{child_type,qu… |
...
| dense_vector quantization / memory-efficient vector storage (config default_quantization: none/scalar8/binary, advertised 4-32x memory reduction) | missing | `crates/xerj-vector/src/quantizer.rs` | ⚠️ scalar8 PARTIAL (`int8_hnsw` serving path shipped — kNN scores against 1-byte-per-dim codes, measured recall@10 ≈ 0.998; see ROADMAP "Real scalar8 vector quantization"). It buys PRECISION, not memory: the scan reads the f32 vector from `_source` and quantizes per query, so there is no smaller working set and the earlier "~4× smaller working set" claim here was wrong — [#392](https://github.com/xerj-org/xerj/issues/392) tracks making it true. `binary` (1-bit) is still rejected at startup and `scalar4`/offload remain future work, so this row is only partially closed. Original finding: REAL gap (dead scaffold with a silently-inert config knob). The quantizer.rs module itself is fully implemented and unit-tested in isolation (NoneQuantizer, Sca… |
---

...
| dense_vector quantization / memory-efficient vector storage (config default_quantization: none/scalar8/binary, advertised 4-32x memory reduction) | missing | `crates/xerj-vector/src/quantizer.rs` | ✅ scalar8 FIXED (`int8_hnsw` serving path shipped — kNN scores against 1-byte-per-dim codes, ~4× smaller working set, measured recall@10 ≈ 0.99; see ROADMAP "Real scalar8 vector quantization"). `binary` (1-bit) is still rejected at startup and `scalar4`/offload remain future work, so this row is only partially closed. Original finding: REAL gap (dead scaffold with a silently-inert config knob). The quantizer.rs module itself is fully implemented and unit-tested in isolation (NoneQuantizer, Sca… |
---
